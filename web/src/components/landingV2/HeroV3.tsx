...
function TypedText({ text, firstLineNoWrap }: { text: string; firstLineNoWrap?: boolean }) {
  return (
    <>
      {text.split("\n").map((line, i) => (
        <Fragment key={i}>
          {i > 0 && <br />}
          <span className={i === 0 ? (firstLineNoWrap ? s.heroHeadlineFirstLine : undefined) : s.heroHeadlineSecondLine}>
            {line}
          </span>
        </Fragment>
      ))}
...
const CONTENT = {
  candidate: {
    ...
    sub: (
      <>
        You don&apos;t apply. Mitra represents you — and introduces you{" "}
        <strong>directly to the founder</strong> who is already expecting the call.
      </>
    ),
  ...
  company: {
    ...
    sub: (
      <>
        Brief us once. Mitra learns your bar from every pass and every hire —{" "}
        <strong>shortlists get sharper automatically.</strong>
      </>
    ),
...
export function HeroV3({ audience }: { audience: V2Audience }) {
    ...
    <section className={s.hero}>
      ...
      <div className={s.heroInner}>
        <div className={s.heroBody} key={audience}>
          ...
          <div className={`${s.heroChip} ${s.fadeUp}`} style={{ "--anim-delay": "0ms" } as React.CSSProperties}>
          ...
          <h1 className={s.heroHeadline} aria-label={c.headlineText.replace(/\n/g, " ")}>
